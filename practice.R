...
#help(trainControl)
evalData <- featureSelectedData[,!names(featureSelectedData) %in% c('c1','c3')]
evalFormula <- as.formula(paste("c2~",paste(names(evalData[,names(evalData) != c('c2')]), collapse='+'), sep=""))

trainControl <- trainControl(method='repeatedcv', number =100, selectionFunction = "oneSE",allowParallel = TRUE)
mtryGrid <- expand.grid(mtry = 100)

rfTune <- train(evalFormula, data = evalData, method="rf",ntree = 1000, metric='kappa', tuneGrid=mtryGrid,trControl=trainControl)
